...
interface Props {
  ...
  /** When complete and remounted (e.g. back on home), start collapsed. */
...
export function AnalysisProgress({
  ...
  // Stay open while building / just finished; collapse by default when returning later.
  ...
  const canCollapse = !active && Boolean(metrics)
    ...
    : cached
      ...
      : expanded
        ...
        : `${formatElapsed(elapsedMs)} · ${
            cached ? '0 tokens (cached)' : `${formatTokens(totalTokens)} tokens`
          } — click to expand`

  return (
    <div
      className={`analysis-progress ${active ? 'is-active' : 'is-complete'}${
        canCollapse ? (expanded ? ' is-expanded' : ' is-collapsed') : ''
      }`}
      role="status"
      aria-live="polite"
    >
      <div className="analysis-progress-top">
        {canCollapse ? (
          <button
            type="button"
            className="analysis-progress-toggle"
            aria-expanded={expanded}
            aria-controls="analysis-progress-details"
            onClick={() => setExpanded((value) => !value)}
          >
            <span className="analysis-progress-title-row">
            ...
            <span className="analysis-progress-toggle-end">
              <span className="analysis-progress-percent" aria-hidden="true">
                <em>{displayPercent}</em>
                <span>%</span>
              </span>
              <span className="analysis-progress-chevron" aria-hidden="true" />
            </span>
          </button>
        ) : (
          <>
      ...
      {showDetails && (
        <div id="analysis-progress-details" className="analysis-progress-details">
          {mode === 'analyze' && (
            ...
            <div className="analysis-token-detail">
              ...
              {!cached && totalTokens != null && totalTokens > 0 ? (
                <>
                  ...
                  >
                    <span
                      className="analysis-token-split-prompt"
                    ...
                    <li>
                      ...
                      <div>
                        ...
                        <span>
                          ...
                          {formatWords(totalTokens)
                            ? ` (${formatWords(totalTokens)} equivalent)`
                            : ''}
                ...
                <p className="analysis-token-cached-note">
                  This response came from cache, so the insight agent did not run again and token
                  usage stayed at zero. Use Refresh to pull a new brief and spend new tokens.
                </p>
              )}
            </div>
          )}
