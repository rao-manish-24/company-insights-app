...
from app.core.deps import get_analysis_service, get_email_service
...
async def analyze_company(
    ...
    logger.info(
        "Analyze requested company=%r force_refresh=%s send_email=%s ip=%s",
    ...
    if not payload.force_refresh:
        cached = service.peek_cache(payload.company_name)
        if cached:
    ...
    result = await service.analyze(payload.company_name, force_refresh=payload.force_refresh)
    logger.info(
    ...
    if should_email:
        row = service.get_by_id(result.id)
        if row:
...
async def email_analysis(
    ...
    row = service.get_by_id(analysis_id)
    if not row:
...
def search_analyses(
    ...
    logger.info("Search analyses q=%r matches=%s", q, len(rows))
    return [CompanyAnalysisListItem.model_validate(row, from_attributes=True) for row in rows]
...
def get_analysis(
    ...
    if not row:
        raise NotFoundError("Analysis not found")
    logger.info("Fetched analysis id=%s company=%r", analysis_id, row.company_name)
